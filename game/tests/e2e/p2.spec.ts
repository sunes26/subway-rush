...
test.describe('S18-4/5 엔딩 도감', () => {
  test('C 를 누르면 14칸이 열리고 미해금은 ??? 다', async ({ page }) => {
    ...
    await expect(page.locator('#collection .cell.locked')).toHaveCount(14)
    await expect(page.locator('#collection .sub')).toContainText('0 / 14')
  ...
  test('엔딩에 도달하면 기록되고 새로고침 후에도 남는다', async ({ page }) => {
    ...
    await expect(page.locator('#collection .sub')).toContainText('1 / 14')
  })
