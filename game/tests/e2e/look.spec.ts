...
test('포인터 락이 걸리고 마우스가 시선을 돌린다', async ({ page }) => {
  ...
  // ⚠ **"입력이 없으면 시선이 정지한다"는 여기서 검증하지 않는다.**
  //   1e-6 단언을 두고 있었는데, 클릭만 하고 마우스를 안 움직인 채 300 ms 구간의
  //   yaw 변화를 재 보니 같은 빌드에서 0.000 · 0.024 · 1.219 rad 로 실행마다 널뛰었다.
  //   포인터 락이 걸릴 때 브라우저가 커서를 워프시키면서 만드는 델타가 환경마다
  //   다르게 들어오기 때문이고, 씬이 무거워질수록(지상 가로·차량) 소진이 늦어져
  //   경계선을 넘는다. 필터가 정상인데도 깨지는 단언은 회귀 감지 구실을 못 한다.
  //
  //   이월·워프·스파이크·히치는 전부 `tests/unit/look.test.ts` 가 본다 —
  //   "락 직후 SETTLE_MS 안의 이벤트는 버린다", "락이 풀렸다 다시 걸리면 잔여 델타가
  //   새 세션으로 새지 않는다" 등. 현상을 재현할 수 있는 쪽에 두는 게 맞다.
  //   (파일 첫머리의 스파이크 단언을 뺀 것과 같은 판단이다.)
