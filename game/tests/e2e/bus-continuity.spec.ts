...
test('전환 구간 — 화면 같은 자리가 버스의 어느 구간을 보나', async ({ page }) => {
  ...
  for (const t of [SHOT.door - 200, 5000, 5400, 5800, 6200, INTRO_MS - 60]) {
    ...
    const r = await page.evaluate(() => {
      ...
      const camY = -p.cam[2]
      /**
       * ★ 레이 **하나**로 재면 흔들린다(같은 코드에서 3.40 / 3.68 을 봤다).
       *   화면 오른쪽 대역 다섯 점을 쏴 맞은 x 의 **중앙값**을 쓴다 — 한 점이
       *   버스를 놓쳐도 지표가 튀지 않는다.
       */
      const xs: number[] = []
      for (const [a2, b2] of [[0.40, -0.05], [0.48, -0.12], [0.55, -0.15],
                              [0.62, -0.18], [0.70, -0.22]] as [number, number][]) {
        const h = window.__game!.pick(a2, b2).find((v: any) => /BUS_/.test(v.name))
        if (h) xs.push(h.point[0] as number)
      }
      xs.sort((p2, q2) => p2 - q2)
      return {
        gap: +(camY - 21.73).toFixed(2),         // 외피 북면까지
        x: xs.length ? xs[Math.floor(xs.length / 2)]! : null,
        hit: xs.length ? `${xs.length}/5점` : '(안 맞음)',
      }
    })
   ...
   * 옮기기로 할 때 같이 내린다.
   *
   * ⚠ ★ **지표를 바꿨으므로 위 수치와 지금 값은 비교할 수 없다.**
   *
   *   예전에는 레이 **하나**로 쟀다. 그래서 프레임 타이밍에 따라 흔들렸고(같은
   *   코드에서 3.40 / 3.68), 카메라가 멀어지면 그 한 점이 버스를 **놓쳐 표본이
   *   빠졌다**(6200·6440ms). 표본이 빠지면 범위가 좁아 보여 수치가 좋아진 것처럼
   *   나온다 — 지표가 스스로를 속인 것이다.
   *
   *   지금은 오른쪽 대역 **다섯 점의 중앙값**을 쓴다. 한 점이 놓쳐도 지표가 안 튀고,
   *   실제로 놓치는 시점이 0 이 됐다. 대신 버리던 표본이 들어와 값이 4.7 로 올라갔다 —
   *   **기하가 나빠진 것이 아니라 재는 법이 정직해진 것이다.**
   *
   *   방지선도 새 지표 기준으로 다시 잡는다. 5.2 는 실측 4.71 위에 여유를 둔 값이다.
   *   목표치를 더 낮추려면 `SPAWN` 을 버스 쪽으로 당겨야 한다(범위 밖).
   */
  expect(sweep, `화면 같은 자리가 버스를 ${sweep.toFixed(2)}m 훑는다 (회귀)`).toBeLessThan(5.2)
})
