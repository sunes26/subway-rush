/**
 * 차도 규칙 — 적신호에도 건너지만 차에 치이면 그 자리에서 끝난다(E-18).
 *
 ...
 * 여기서는 **배선이 살아 있는가**만 본다 — 차선에 서 있으면 결국 즉사 엔딩이 뜬다.
 *
...
test('차선 한복판에 서 있으면 차에 치여 즉사 엔딩(E-18)이 뜬다', async ({ page }) => {
  await boot(page)
  ...
  await expect.poll(async () => {
    ...
    return { phase: s.phase, endingId: s.endingId }
  }, { timeout: 90_000, intervals: [500] }).toEqual({ phase: 'ended', endingId: 'E-18' })
})
...
test('인도에 서 있으면 아무 일도 없다 — 오탐이 없다', async ({ page }) => {
  ...
  const s = await page.evaluate(() => window.__game!.state())
  expect(s.phase, '인도에 서 있는데 즉사 판정이 났다').toBe('playing')
})
