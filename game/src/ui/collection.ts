 ...
 * ■ 화면 구조 — **목록 + 상세** 두 칸
 *
 * 예전 판은 칸마다 칸마다 제목·대사·횟수·최고기록을 다 적었다.
 * 정보는 다 있었지만 **어느 것도 눈에 안 들어왔다** — 열여섯 칸 × 4줄이 같은 크기로
 * 깔리면 그건 목록이 아니라 벽이다. 슬롯은 "무엇이 있고 무엇이 없나"만 말하고,
 * 자세한 것은 **고른 하나**에 대해서만 옆에서 말한다.
 *
 *   왼쪽  16칸 그리드(4 × 4) — 상태(잠금/해금/선택)가 한눈에
 ...
 * 도감은 판정표가 아니라 목록이므로 번호순으로 고정한다.
 * (양심 축 폐지로 E-07·E-10 이 빠져 번호가 이어지지 않는다 — 번호 연속화는
 *  세이브 키 마이그레이션이 딸려 별건이다. 여기서는 있는 번호를 그대로 쓴다.) 순서가 판마다
...
/**
 * 한 줄에 몇 칸인가 — 방향키 상하 이동이 이 값을 쓴다. **CSS 의 열 수와 같아야 한다.**
 *
 * ■ ★ 3 → 4 로 바꿨다 — 엔딩이 18종에서 **16종**이 됐다
 *
 * upstream 이 양심(conscience) 축을 없애며 E-07·E-10 이 사라졌다. 3열로 두면
 * 6행 중 마지막 행에 **빈 칸 둘**이 남는다. 도감은 "몇 개 중 무엇을 봤나"를 한눈에
 * 보여 주는 화면이라, 아무것도 아닌 빈 자리가 **아직 못 깐 칸으로 오해**된다.
 *
 * 16 = 4 × 4 라 딱 맞는다. 열이 늘어 칸이 좁아지지만 제목 글자는 `clamp` 이 받는다.
 *
 * ⚠ 이 값과 CSS 의 `grid-template-columns` 가 어긋나면 **방향키 위/아래가 엉뚱한
 *   칸으로 뛴다.** 눈에 잘 안 띄는 종류라 여기 같이 적어 둔다.
 */
const COLS = 4
 ...
 *
 * 한때 여기에 한마디까지 넣었다. 정보는 늘었는데 칸마다 네 줄씩 깔리니 글자를
...
const slotHtml = (e: EndingDef, rec: EndingRecord | undefined, sel: boolean): string => {
   ...
   * 그 말을 더 빨리 한다. 글자가 하나 줄면 칸이 조용해지고, 목록을 훑을 때
...
export const createCollection = (mount: HTMLElement): Collection => {
  ...
  /**
   * 칸 클릭 — 위임으로 받는다. 매 렌더마다 칸마다 리스너를 다는 대신
