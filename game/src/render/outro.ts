 ...
 *
 * 탄 채로 끝나는 실패가 여럿 있다: E-09(부정승차 적발) · E-11(에스컬레이터 참사) 은
 * 우선순위가 E-04·E-02 보다 높아서 **열차에 타고도** 그쪽이 뜬다. `boarded` 만 보고
 * 성공 컷을 틀면 부정승차로 적발된 판에 한강 일출이 뜬다.
 * 그래서 목록으로 잠근다 — 엔딩이 늘면 여기 한 줄을 의식적으로 더해야 한다.
 *
 * (E-10「양심 파산」은 upstream 이 양심 축을 없애며 함께 사라졌다. 이 목록은
 *  **넣는 쪽**을 적으므로 엔딩이 없어져도 조용히 깨지지 않는다 — 그게 이 방식의 값이다.)
 *
