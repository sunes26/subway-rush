...
export const poseAt = (tMs: number): IntroPose => {
  ...
  if (t < SHOT.phone) {
    const p = framed(
      /**
       * ★ 뒤로·오른쪽으로·위로 뺐다 — 몸통이 화면 대부분을 먹고 있었다.
       *
       * 0.37~0.48m 에서는 어깨와 팔이 프레임의 2/3 를 채워, 폰이 초점이 아니라
       * 여백에 놓인 물건이 됐다. 0.62m 로 물러나고 남쪽(인물의 오른쪽)으로 더
       * 돌아서 **상체 일부 · 팔 · 손 · 폰 · 좌석과 창**이 한 화면에 들어오게 한다.
       */
      [SEAT.x + 0.18, SEAT.y - 0.68, 1.52],
      [SEAT.x + 0.32, SEAT.y - 0.52, 1.36],
      /**
