 ...
 *   그대로 걸렸다. 2680 이면 컷까지 120ms 라 "일어서기 시작"만 보이고 몸이
 *   렌즈를 덮기 전에 장면이 바뀐다.
 */
export const STAND_MS = 2680
 ...
 */
/**
 * 앉은 자리. x 를 좌석 중심에서 **9cm 서쪽**으로 둔다 — 등이 등받이에 닿는다
 * (등받이 앞면이 중심에서 0.19 서쪽이다). 5cm 로는 등과 등받이가 떠 보였다.
 */
const SEAT = { x: -61.37, y: 21.20 } as const
/** 뒷문 안쪽 */
