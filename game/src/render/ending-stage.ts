...
export const buildEndingStage = (): EndingStage => {
  ...
  return {
    ...
    sync({ x, yOff, tunnel: tunnelK, scroll, led: ledK, wrong, glow, flash, dim }) {
       ...
       *
       * ⚠ **컷을 늘릴 때마다 여기를 같이 내려야 한다.** 흐른 거리는 시간에 비례하므로
       *   길이를 늘리면 이음매가 화면 쪽으로 다가온다. 지금까지의 이력:
       *
       *     5.2초 · scroll 4.2 → 0.030/0.040 → 0.126 / 0.168
       *     7.0초 · scroll 6.0 → 0.030/0.034 → 0.180 / 0.204
       *     8.5초 · scroll 7.5 → 0.025/0.028 → 0.188 / 0.210   ← 지금
       *
       *   비율을 안 내렸다면 8.5초에서 지옥이 0.255 로 **한계를 넘는다.**
       *
       * 지옥이 여전히 12% 빠르다 — 같은 열차인데 더 급해 보이는 것이 이 엔딩의 불안이다.
       */
      dawnTex.offset.x = scroll * 0.025
      hellTex.offset.x = scroll * 0.028
    },
  }
}
