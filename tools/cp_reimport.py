...
blender -b --factory-startup --python cp_reimport.py -- <glb> <fbx> <report.json>
"""
...
def inspect(tag):
    ...
    if len(meshes) != 2:
        fail('%s: expected 2 meshes (body + carrier), got %s' % (tag, [m.name for m in meshes]))
    if len(arms) != 1:
    ...
    if "Prop.Case" not in [b.name for b in arm.data.bones]:
    ...
    ph = next((m for m in meshes if m.name.startswith("PR_Carrier")), None)
    if ph is None:
        fail("%s: carrier prop missing" % tag)
    else:
        d["carrier"] = {"name": ph.name, "verts": len(ph.data.vertices),
        ...
        if ph.parent_bone != "Prop.Case":
            fail("%s: carrier lost Prop.R bone parent (got %r)" % (tag, ph.parent_bone))
