...
def _sign_face(sb, sw, sg, sy, x, sgn, cy, wid, z0, z1, kind):
    ...
    # 화살표 — 개찰구 사인은 **양면 모두 직진**이다(서면 "승강장"은 동쪽,
    # 동면 "나가는 곳 · 환승"은 서쪽 — 둘 다 통로를 따라 계속 가면 된다).
    # 직진은 좌우가 아니라 **위쪽 화살표(↑)** 로 그린다. 좌우로 그리면
    # 통로를 따라가라는 안내가 옆을 가리켜 엉뚱하게 읽힌다.
    ax = cy + sgn * wid * 0.36
    h = (z0 + z1) / 2
    pts = [(ax - 0.085, h - 0.160), (ax + 0.085, h - 0.160), (ax + 0.085, h + 0.010),
           (ax + 0.170, h + 0.010), (ax, h + 0.185), (ax - 0.170, h + 0.010),
           (ax - 0.085, h + 0.010)]
    n0 = fx + sgn * d
