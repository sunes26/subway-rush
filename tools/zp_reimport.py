...
blender -b --factory-startup --python zp_reimport.py -- <glb> <fbx> <report.json>
"""
...
def inspect(tag):
    ...
    if len(meshes) != 2:
        fail('%s: expected 2 meshes (body + phone), got %s' % (tag, [m.name for m in meshes]))
    if len(arms) != 1:
    ...
    ph = next((m for m in meshes if m.name.startswith("PR_Phone")), None)
    if ph is None:
        fail("%s: phone prop missing" % tag)
    else:
        d["phone"] = {"name": ph.name, "verts": len(ph.data.vertices),
        ...
        if ph.parent_bone != "Prop.R":
            fail("%s: phone lost Prop.R bone parent (got %r)" % (tag, ph.parent_bone))
