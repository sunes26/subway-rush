"""CP GLB/FBX 익스포트. 기존 저장소 규약(export_apply=False, NLA 트랙, 리프본 없음,
정점당 영향 4, 리그 위치 0)을 따른다.

blender -b cp_character.blend --python cp_export.py -- <glb_path> <fbx_path>
"""
...
REPORT = os.environ.get("CP_REPORT", "/tmp/cp_export_report.json")
rep = {}
...
if not any(m.type == 'ARMATURE' for m in mesh.modifiers):
    raise RuntimeError("Armature modifier missing on CP_Character")
if prop.parent_bone != "Prop.Case":
    raise RuntimeError("carrier not parented to Prop.Case")
...
if len(ad.nla_tracks) != len(CP_ACTIONS):
...
print("CP EXPORT OK", os.path.getsize(GLB), os.path.getsize(FBX))
