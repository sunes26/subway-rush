...
BOOTH_Y0, BOOTH_Y1 = 28.30, Y1 - T      # 부스 깊이 1.50
PART_T = 0.04                            # 칸막이 두께
...
M_GLASS = bpy.data.materials["ST_GLASS"]     # 거울 — 유리로 렌더된다
...
tile, part, fixt, cntr, metal, glass, light = [], [], [], [], [], [], []
...
part.append((40.70, Y0 + T, FLOOR, 40.90, 27.50, FLOOR + 2.00))     # 가림벽

for (a, b) in booths(MX0, 40.70, 2):                                # 부스 2칸
    ...
    fixt.append((a + 0.30, BOOTH_Y1 - 0.62, FLOOR, b - 0.30, BOOTH_Y1 - 0.10, FLOOR + 0.40))    # 변기
    fixt.append((a + 0.28, BOOTH_Y1 - 0.22, FLOOR + 0.40, b - 0.28, BOOTH_Y1 - 0.02, FLOOR + 0.78))
part.append((40.70 - PART_T, BOOTH_Y0, FLOOR + 0.15, 40.70, BOOTH_Y1, FLOOR + 1.90))
...
cntr.append((MX0 + 0.20, Y0 + T, COUNTER_Z - 0.05, MX0 + 2.20, COUNTER_Y1, COUNTER_Z))
for i in range(3):                                                   # 세면기 볼
    x = MX0 + 0.55 + i * 0.55
...
glass.append((MX0 + 0.20, Y0 + T, COUNTER_Z + 0.15, MX0 + 2.20, Y0 + T + 0.02, COUNTER_Z + 0.95))

# ── 여자 화장실 ───────────────────────────────────────────────
FX0, FX1 = DIV_M_F + T / 2, DIV_F_A - T / 2
part.append((46.50, Y0 + T, FLOOR, 46.70, 27.50, FLOOR + 2.00))     # 가림벽

for (a, b) in booths(FX0, 46.50, 3):                                # 부스 3칸
    part.append((a, BOOTH_Y0, FLOOR + 0.15, a + PART_T, BOOTH_Y1, FLOOR + 1.90))
    part.append((a + 0.06, BOOTH_Y0, FLOOR + 0.15, b - 0.06, BOOTH_Y0 + PART_T, FLOOR + 1.90))
    fixt.append((a + 0.30, BOOTH_Y1 - 0.62, FLOOR, b - 0.30, BOOTH_Y1 - 0.10, FLOOR + 0.40))
    fixt.append((a + 0.28, BOOTH_Y1 - 0.22, FLOOR + 0.40, b - 0.28, BOOTH_Y1 - 0.02, FLOOR + 0.78))
part.append((46.50 - PART_T, BOOTH_Y0, FLOOR + 0.15, 46.50, BOOTH_Y1, FLOOR + 1.90))

cntr.append((FX0 + 0.20, Y0 + T, COUNTER_Z - 0.05, FX0 + 2.75, COUNTER_Y1, COUNTER_Z))
for i in range(4):
    x = FX0 + 0.55 + i * 0.55
    fixt.append((x - 0.20, Y0 + T + 0.08, COUNTER_Z - 0.16, x + 0.20, COUNTER_Y1 - 0.08, COUNTER_Z - 0.05))
    metal.append((x - 0.02, Y0 + T + 0.04, COUNTER_Z, x + 0.02, Y0 + T + 0.10, COUNTER_Z + 0.16))
glass.append((FX0 + 0.20, Y0 + T, COUNTER_Z + 0.15, FX0 + 2.75, Y0 + T + 0.02, COUNTER_Z + 0.95))

# ── 다목적 화장실 ─────────────────────────────────────────────
AX0, AX1 = DIV_F_A + T / 2, X1 - T
fixt.append((AX0 + 0.30, Y1 - T - 0.62, FLOOR, AX0 + 0.68, Y1 - T - 0.10, FLOOR + 0.44))
fixt.append((AX0 + 0.28, Y1 - T - 0.22, FLOOR + 0.44, AX0 + 0.70, Y1 - T - 0.02, FLOOR + 0.82))
...
glass.append((AX1 - 0.70, Y0 + T, COUNTER_Z + 0.15, AX1, Y0 + T + 0.02, COUNTER_Z + 0.95))
...
emit("Z2_OBJ14_mirror", glass, M_GLASS)
...
plate.append((43.60, SIGN_Y, FLOOR + 1.86, 45.80, Y0, FLOOR + 2.72))   # 큰 안내 사인
...
text("Z2_OBJ14_txt_kr", "화 장 실", 44.70, FLOOR + 2.44, 0.20, "TXT_WHITE")
text("Z2_OBJ14_txt_en", "Restroom", 44.70, FLOOR + 2.10, 0.11, "TXT_WHITE")
...
print(f"화장실 재건 — 벽 {len(tile)} · 칸막이 {len(part)} · 위생기구 {len(fixt)} 박스")
