...
# 높이는 그대로 두고 **반경만** 줄인다. 아래로 갈수록 많이 깎아(바닥 −10%,
# 목 −4.5%) 배가 덜 부푼다 — 통통해 보이던 원인은 높이가 아니라 밑동 부피였다.
BM_RINGS = [(0.0000, 0.01422), (0.0016, 0.01623), (0.0042, 0.01754),
            (0.0090, 0.01799),
            (0.0180, 0.01861), (0.0286, 0.01882), (0.0336, 0.01845),
            (0.0390, 0.01753), (0.0450, 0.01579), (0.0505, 0.01365),
            (0.0550, 0.01105), (0.0578, 0.00955)]
#             바닥 ─── 배 ───┬── 초록 인쇄 띠 ──┬── 어깨 ── 목(밝은 면)
_rows = [0, 0, 0, 0, 0, 3, 3, 3, 0, 1, 1]
_p = [revolve("bm_body", BM_RINGS, BM_SEG,
              [M_BM, M_BM_LIT, M_BM_CAP, M_BM_GRN], _rows)]
_p.append(revolve("bm_cap", [(0.0578, 0.00984), (0.0588, 0.01127),
                             (0.0618, 0.01108), (0.0626, 0.01031)],
                  BM_SEG, M_BM_CAP))
...
for _r in range(CH_ROW):
    for _c in range(CH_COL):
        ...
        _b = plate("ch_blk%d%d" % (_r, _c), _CW, _CHH, 0.0016, CH_D + 0.0014,
...
# 몸통 밖으로 나가는 살은 한쪽 0.65mm 뿐이다 — 판 두께(10mm)의 6.5%.
CH_FD = CH_D + 0.0013
...
SD_SEG = 18
SD_R = 0.0156          # 지름 34 → 31.2mm. 캔은 가늘어야 캔이다
SD_RINGS = [(0.0000, 0.01212), (0.0026, 0.01506), (0.0056, SD_R),
            (0.0668, SD_R), (0.0730, 0.01414), (0.0788, 0.01166),
...
# 정면 실루엣(70×98)은 건드리지 않고 **두께만** 42 → 36mm (−14%).
# 봉지의 부풀음은 남기되, 옆에서 봤을 때 베개가 아니라 봉지로 보인다.
SN_W, SN_H, SN_D = 0.070, 0.098, 0.0360
